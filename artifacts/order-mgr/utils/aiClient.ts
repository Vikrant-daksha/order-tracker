/**
 * Gemini AI Client — Direct Google Generative Language API
 *
...
export async function sendMessage(
  ...
  if (!apiKey) {
    return {
      reply:
        "Gemini API key is not configured.\n\n• Please make sure `EXPO_PUBLIC_GEMINI_KEY` is added to your `.env` file.\n• Restart the Metro bundler with `npx expo start -c` to reload your environment variables.",
      tokensUsed: 0,
      limitReached: false,
    };
  }
  ...
  for (let iteration = 0; iteration < 5; iteration++) {
    ...
    const reply =
      textParts.join("").trim() ||
      "I'm not sure how to help with that. Try asking about your orders, revenue, or expenses!";
