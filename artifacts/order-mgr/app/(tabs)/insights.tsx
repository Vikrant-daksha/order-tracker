...
function sumOutstanding(orders: Order[]) {
  return orders.reduce(
    (s, o) => s + Math.max(0, (o.price || 0) - (o.amountPaid || 0)),
    0,
  );
}
...
function buildMonthChart(
  ...
  return weeks.map((w) => ({
    label: w.label,
    primary: filterOrdersByRange(orders, w.from, w.to).reduce(
      (s, o) => s + (o.amountPaid || 0),
      0,
    ),
    secondary: filterExpensesByRange(expenses, w.from, w.to).reduce(
      (s, e) => s + e.amount,
      0,
    ),
  }));
...
export default function InsightsScreen() {
  ...
  const { orders, expenses, addExpense, updateExpense, deleteExpense } =
    useDatabase();
  ...
  const { fromDate, toDate, periodLabel, weekStart } = useMemo(() => {
    ...
    if (tab === "Week") {
      ...
      return {
        fromDate: toDateStr(base),
        toDate: toDateStr(end),
        periodLabel: weekLabel(base),
        weekStart: base,
      };
    }
    if (tab === "Month") {
      ...
      return {
        ...
        toDate: toDateStr(last),
        periodLabel: monthLabel(d.getFullYear(), d.getMonth()),
        weekStart: d,
      };
    }
    return {
  ...
  const filteredOrders = useMemo(
    () =>
      tab === "AllTime"
        ? orders
        : filterOrdersByRange(orders, fromDate, toDate),
    [orders, tab, fromDate, toDate],
  );
  const filteredExpenses = useMemo(
    () =>
      tab === "AllTime"
        ? expenses
        : filterExpensesByRange(expenses, fromDate, toDate),
    [expenses, tab, fromDate, toDate],
  );
  ...
  const chartData: BarData[] = useMemo(() => {
    if (tab === "Week")
      return buildWeekChart(filteredOrders, filteredExpenses, weekStart);
    if (tab === "Month") {
  ...
  const sourceCounts = useMemo(
    () => getSourceCounts(filteredOrders),
    [filteredOrders],
  );
  const topProducts = useMemo(
    () => getTopProducts(filteredOrders),
    [filteredOrders],
  );
      ...
      >
        ...
        >
          {(["Week", "Month", "AllTime"] as Tab[]).map((t) => (
            <Pressable
              ...
              style={[
                styles.tabBtn,
                tab === t && { backgroundColor: PRIMARY_COLOR },
              ]}
            >
              <Text
                style={[
            ...
            <Pressable
              onPress={() => {
                if (tab === "Week" && weekOffset < 0)
                  setWeekOffset((w) => w + 1);
                if (tab === "Month" && monthOffset < 0)
                  setMonthOffset((m) => m + 1);
              }}
              style={[
                styles.navBtn,
                { backgroundColor: colors.card, borderColor: colors.border },
                ((tab === "Week" && weekOffset >= 0) ||
                  (tab === "Month" && monthOffset >= 0)) &&
        ...
        <View style={styles.kpiGrid}>
          ...
          <View style={styles.kpiRow}>
            <KPICard
              ...
              icon="activity"
              accent={
                kpis.netProfit >= 0 ? colors.deliveredText : colors.overdueText
              }
          ...
          <View style={styles.kpiRow}>
            ...
            <KPICard
              ...
              icon="clock"
              accent={
                kpis.overdue > 0 ? colors.overdueText : colors.mutedForeground
              }
            />
          </View>
        </View>
        ...
        >
          ...
          <MiniBarChart
            ...
            secondaryColor={EXPENSE_COLOR}
          />
          {chartData.every(
            (d) => d.primary === 0 && (d.secondary ?? 0) === 0,
          ) && (
            <Text
            ...
            {sourceCounts.map(([source, count]) => {
              ...
              return (
                <View key={source} style={styles.sourceRow}>
                  <View
                    style={[styles.sourceDot, { backgroundColor: barColor }]}
                  />
                  <Text
            ...
            {topProducts.map(([name, count], i) => (
              <View key={name} style={styles.productRow}>
                <View
                  style={[
                    styles.rank,
                    { backgroundColor: i === 0 ? PRIMARY_COLOR : colors.muted },
                  ]}
                >
                  <Text
                    style={[
            ...
            {sortedExpenses.map((exp) => (
              ...
              >
                <View
                  style={[
                    styles.catDot,
                    { backgroundColor: EXPENSE_COLOR + "22" },
                  ]}
                >
                ...
                <View style={styles.expContent}>
                  <Text
                    style={[styles.expCategory, { color: colors.foreground }]}
                  >
                    {exp.category}
                  </Text>
                  {!!exp.note && (
                ...
                <View style={styles.expRight}>
                  <Text style={[styles.expAmount, { color: EXPENSE_COLOR }]}>
                    −{fmt(exp.amount)}
                  </Text>
                  <Text
                    style={[styles.expDate, { color: colors.mutedForeground }]}
                  >
                    {exp.date}
                  </Text>
                </View>
