...
const DEFAULT_CATEGORIES: ExpenseCategory[] = [
  'Supplies', 'Packaging', 'Shipping', 'Marketing', 'Equipment', 'Other',
];
...
export function AddExpenseModal({ visible, onClose, onSave, initialExpense }: AddExpenseModalProps) {
  ...
  useEffect(() => {
    if (visible) {
      ...
      Animated.timing(slideAnim, { toValue: 500, useNativeDriver: true, duration: 200 }).start();
    }
  }, [visible]);
  ...
  function handleSave() {
    ...
    if (!amt || amt <= 0) return;
    onSave({ amount: amt, category, note: note.trim(), date: date || todayStr() });
    onClose();
  }
          ...
          <ScrollView keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
            ...
            <View style={styles.field}>
              ...
              <View style={styles.catWrap}>
                ...
                {addingCustom ? (
                  <View style={[styles.customInputRow, { borderColor: colors.border, backgroundColor: colors.muted }]}>
                    <TextInput
                    ...
                    />
                    <Pressable onPress={handleAddCustom} style={[styles.customSaveBtn, { backgroundColor: colors.primary }]}>
                  ...
                  <Pressable
                    ...
                    style={[styles.catChip, { borderColor: colors.border, backgroundColor: 'transparent', borderStyle: 'dashed' }]}
                  >
                    <Text style={[styles.catText, { color: colors.mutedForeground }]}>+ Custom</Text>
                  </Pressable>
                )}
              </View>
            </View>
...
const styles = StyleSheet.create({
  ...
  titleRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingBottom: 16 },
  ...
  customInput: { flex: 1, fontSize: 13, fontFamily: 'Inter_500Medium', paddingVertical: 3 },
  customSaveBtn: { width: 26, height: 26, borderRadius: 13, alignItems: 'center', justifyContent: 'center' },
  noteInput: {
    borderRadius: 12,
    borderWidth: 1,
    paddingHorizontal: 14,
